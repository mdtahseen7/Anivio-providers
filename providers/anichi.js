/**
 * Anichi Stream Provider Plugin for Anivio
 *
...
var ANICHI_BASE = 'https://anichi.to';
...
async function searchAnichi(query) {
    var res = await fetch(ANICHI_BASE + '/filter?keyword=' + encodeURIComponent(query), {
        headers: { 'User-Agent': UA, 'Referer': ANICHI_BASE + '/' }
    });
    if (!res.ok) return [];
    var html = await res.text();
    var re = /<a\s+class="name d-title"\s+href="https:\/\/anichi\.to\/(?:watch|anime)\/([^"/]+)(?:\/ep-\d+)?"[^>]*data-jp="([^"]*)"[^>]*>([\s\S]*?)<\/a>/g;
    var m;
    var candidates = [];
    var seen = new Set();
    ...
    if (candidates.length === 0) {
        var re2 = /href="https:\/\/anichi\.to\/(?:anime|watch)\/([^"\/]+)(?:\/ep-\d+)?"/g;
        while ((m = re2.exec(html)) !== null) {
            var slug2 = m[1];
            if (seen.has(slug2)) continue;
            seen.add(slug2);
            var nameM = html.slice(m.index - 3000, m.index + 3000).match(/data-jp="([^"]*)"[^>]*>([\s\S]*?)<\/a>/);
            var jp2 = nameM ? nameM[1].trim() : '';
            var nm2 = nameM ? nameM[2].replace(/<[^>]*>/g, '').trim() : slug2.replace(/-/g, ' ');
            if (!nm2) nm2 = slug2.replace(/-/g, ' ');
            candidates.push({ slug: slug2, jp: jp2, name: nm2 });
        }
    }
    return candidates;
}
...
async function fetchShowEpisodeIndex(slug, cache) {
    ...
    try {
        var watchRes = await fetch(ANICHI_BASE + '/watch/' + slug, {
            headers: { 'User-Agent': UA, 'Referer': ANICHI_BASE + '/' }
        });
        ...
        var epRes = await fetch(ANICHI_BASE + '/ajax/episode/list/' + showIdMatch[1], {
            headers: {
                'User-Agent': UA,
                'X-Requested-With': 'XMLHttpRequest',
                'Referer': ANICHI_BASE + '/watch/' + slug
            }
        });
...
async function getStreams(tmdbId, mediaType, season, episode) {
    try {
        console.log('[anichi] getStreams called: id=' + tmdbId + ' season=' + season + ' ep=' + episode);
        ...
        for (var i = 0; i < queries.length; i++) {
            var found = await searchAnichi(queries[i]);
        ...
        if (candidates.length === 0) {
            console.log('[anichi] No results found on Anichi for ' + tmdbId);
            return [];
        }
        ...
        if (meta.malId) {
            ...
            if (verified) {
                chosen = verified;
                console.log('[anichi] MAL-verified show: ' + chosen.name + ' (' + chosen.slug + ') mal=' + meta.malId);
            } else {
                console.log('[anichi] No MAL match for mal=' + meta.malId + ', using best title match');
            }
        }
        console.log('[anichi] Chosen show: ' + chosen.name + ' (' + chosen.slug + ') for Season ' + targetSeason);
        ...
        if (!targetEpData || !targetEpData.ids) {
            console.log('[anichi] Episode ' + targetEp + ' not found in index');
            return [];
        }

        // Fetch servers for this episode
        var srvRes = await fetch(ANICHI_BASE + '/ajax/server/list?servers=' + encodeURIComponent(targetEpData.ids), {
            headers: {
                'User-Agent': UA,
                'X-Requested-With': 'XMLHttpRequest',
                'Referer': ANICHI_BASE + '/'
        ...
        for (var s = 0; s < serverButtons.length; s++) {
            ...
            try {
                var embedRes = await fetch(ANICHI_BASE + '/ajax/server?get=' + encodeURIComponent(sb.linkId), {
                    headers: {
                        'User-Agent': UA,
                        'X-Requested-With': 'XMLHttpRequest',
                        'Referer': ANICHI_BASE + '/'
                ...
                if (embedUrl.indexOf('megaplay') !== -1 || embedUrl.indexOf('stream') !== -1) {
                    ...
                    streams.push({
                        name: 'Anichi (' + sb.sName + ' - ' + labelType + ')',
                        title: 'Anichi · ' + sb.sName + ' · ' + labelType + ' · ' + epLabel,
                ...
                console.warn('[anichi] server resolve error: ' + (err && err.message));
            }
        }

        console.log('[anichi] returning ' + streams.length + ' streams');
        return streams;
    } catch (e) {
        console.error('[anichi] Fatal error: ' + (e && e.message));
        return [];
    }
}
    ...
    return [
        {
            ...
            type: 'text',
            title: 'Provider Name',
            description: 'Display name for Anichi streams.',
            default: 'Anichi'
        }
    ];
}
