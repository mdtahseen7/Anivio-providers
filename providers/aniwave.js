/**
 * AniWave Stream Provider Plugin for Anivio
 *
...
async function searchAniWave(query) {
    var res = await fetch(ANIWAVE_BASE + '/filter?keyword=' + encodeURIComponent(query), {
        headers: { 'User-Agent': UA, 'Referer': ANIWAVE_BASE + '/' }
    });
    if (!res.ok) return [];
    var html = await res.text();
    var re = /<a\s+class="name d-title"\s+href="https:\/\/animewave\.to\/watch\/([^"/]+)(?:\/ep-\d+)?"[^>]*data-jp="([^"]*)"[^>]*>([\s\S]*?)<\/a>/g;
    var m;
    var candidates = [];
    var seen = new Set();
...
async function fetchShowEpisodeIndex(slug, cache) {
    ...
    try {
        var watchRes = await fetch(ANIWAVE_BASE + '/watch/' + slug, {
            headers: { 'User-Agent': UA, 'Referer': ANIWAVE_BASE + '/' }
        });
        ...
        var epRes = await fetch(ANIWAVE_BASE + '/ajax/episode/list/' + showIdMatch[1], {
            headers: {
                'User-Agent': UA,
                'X-Requested-With': 'XMLHttpRequest',
                'Referer': ANIWAVE_BASE + '/watch/' + slug
            }
        });
...
async function getStreams(tmdbId, mediaType, season, episode) {
    try {
        console.log('[aniwave] getStreams called: id=' + tmdbId + ' season=' + season + ' ep=' + episode);
        ...
        for (var i = 0; i < queries.length; i++) {
            var found = await searchAniWave(queries[i]);
        ...
        if (candidates.length === 0) {
            console.log('[aniwave] No results found on AniWave for ' + tmdbId);
            return [];
        }
        ...
        if (meta.malId) {
            ...
            if (verified) {
                chosen = verified;
                console.log('[aniwave] MAL-verified show: ' + chosen.name + ' (' + chosen.slug + ') mal=' + meta.malId);
            } else {
                console.log('[aniwave] No MAL match for mal=' + meta.malId + ', using best title match');
            }
        }
        console.log('[aniwave] Chosen show: ' + chosen.name + ' (' + chosen.slug + ') for Season ' + targetSeason);
        ...
        if (!targetEpData || !targetEpData.ids) {
            console.log('[aniwave] Episode ' + targetEp + ' not found in index');
            return [];
        }

        // Fetch servers for this episode
        var srvRes = await fetch(ANIWAVE_BASE + '/ajax/server/list?servers=' + encodeURIComponent(targetEpData.ids), {
            headers: {
                'User-Agent': UA,
                'X-Requested-With': 'XMLHttpRequest',
                'Referer': ANIWAVE_BASE + '/'
            }
        });
        if (!srvRes.ok) return [];
        ...
        for (var s = 0; s < serverButtons.length; s++) {
            ...
            try {
                var embedRes = await fetch(ANIWAVE_BASE + '/ajax/server?get=' + encodeURIComponent(sb.linkId), {
                    headers: {
                        'User-Agent': UA,
                        'X-Requested-With': 'XMLHttpRequest',
                        'Referer': ANIWAVE_BASE + '/'
                ...
                if (embedUrl.indexOf('megaplay') !== -1 || embedUrl.indexOf('stream') !== -1) {
                    ...
                    streams.push({
                        name: 'AniWave (' + sb.sName + ' - ' + labelType + ')',
                        title: 'AniWave · ' + sb.sName + ' · ' + labelType + ' · ' + epLabel,
                ...
                console.warn('[aniwave] server resolve error: ' + (err && err.message));
            }
        }

        console.log('[aniwave] returning ' + streams.length + ' streams');
        return streams;
    } catch (e) {
        console.error('[aniwave] Fatal error: ' + (e && e.message));
        return [];
    }
}
    ...
    return [
        {
            ...
            type: 'text',
            title: 'Provider Name',
            description: 'Display name for AniWave streams.',
            default: 'AniWave'
        }
    ];
}
